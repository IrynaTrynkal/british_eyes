...
export const OfferCard = ({ data }: { data: DiscountType }) => {
    ...
    return (
        <div className="text-ivory prepc:mx-0 tab:aspect-[435/327] tab:max-w-[435px] prepc:w-[32%] mx-auto aspect-[288/194] max-w-80 overflow-hidden rounded bg-center">
            ...
            <div
                className={`tab:p-3 relative flex w-full flex-col justify-between bg-cover bg-center p-2 ${data.text?.premium ? "tab:h-[265px] h-[154px]" : "h-full"}`}
                style={{
                    backgroundImage: `url(${data.text?.bgimage ?? "/images/main-offer.jpg"})`,
                }}
            >
                <div>
                    <div className="tab:flex-col tab:gap-16 tab:mb-6 pc:mb-3 mb-3 flex justify-between">
                        <h4
                            className={`font-oswald tab:max-w-[288px] max-w-[60%] font-medium uppercase ${data.text?.premium ? "tab:mt-3 tab:text-3xl tab:leading-9 pc:text-[42px] pc:leading-12 mt-2 text-xl" : "pc:text-2xl tab:text-[22px] tab:leading-7"}`}
                        >
                            {data.textLocal?.title}
                        </h4>
                        {data.text?.period && (
                            <p className="font-oswald pc:text-2xl tab:text-xl tab:leading-5 pc:leading-7 text-sm leading-4 font-bold uppercase">
                                {t("discountPeriod", {
                                    date: data.text.period,
                                })}
                            </p>
                        )}
                    </div>
                ...
                {data.text?.cost ? (
                    ...
                    data.text?.icon && (
                        <Image
                            src={data.text.icon}
                            alt={data.textLocal?.title || "discount"}
                            width={77}
                            height={77}
